...
%solveddYALMIP Solve datadriven controller synthesis problem using YALMIP
%
%    * Updated for loop-shaping and robust ellipsoids (Sep 2023)
%   
%   ------------------------------------------------------------------------
...
for mod =  1: nMod
    ...
    if  (sol.satisfyConstraints)
        ...
        % --- Loop Shaping Objective
        
        W_LS = datadriven.utils.resp(objective.LS2.W, W);
        if all(W_LS==0)
            F_LS = [];
        else
            M = datadriven.utils.resp(objective.LS2.Ld, W);
            F_LS = W_LS .* (M.*P - G.*Xf);
        end

        CON = [CON, datadriven.utils.rcone_serialized(PHI,gam,[F_a,F_b,F_LS])];
        % END Hinf
...
%         CON = [CON, datadriven.utils.rcone_serialized(PHI,gam,[F_a,F_b])];
    ...
    % -- Robust Ellipsoid
    gam = 0.5 * (1 + slack(3));
    if ~isempty(parameters.robustEllipsoid)
        X_bar = [Xf, 1j .* Xf];
        for ii = 1:nCon
            A = parameters.robustEllipsoid{ii};
%             CON = [CON, ...
%                 [
%                     A' * A      , X_bar(ii, :)' ;
%                     X_bar(ii, :), PHI(ii)       ;
%                 ] >= 0;
%                 ];
            CON = [CON,  datadriven.utils.rcone_serialized(PHI(ii), gam , X_bar(ii, :) / A)];
        end
...
if strcmp(parameters.solver,'')
    ...
    options = sdpsettings('verbose',0,'solver',parameters.solver);
end
