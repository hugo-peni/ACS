...
%solveddMOSEK Solve datadriven controller synthesis problem using Fusion API
% 
%   ------------------------------------------------------------------------
...
for mod =  1: nMod
    ...
    if  (parameters.robustNyquist)
        ...
        M.constraint(Expr.add(Expr.constTerm(x1_a*XY_c),Expr.mul(Matrix.dense(x1_a),XY)),Domain.greaterThan(1e-5));
        M.constraint(Expr.add(Expr.constTerm(x1_b*XY_c),Expr.mul(Matrix.dense(x1_b),XY)),Domain.greaterThan(1e-5));
    end
